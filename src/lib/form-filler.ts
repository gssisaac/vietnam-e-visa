...
async function runStep(name: string, fn: () => Promise<void>, result: FillResult) {
  try {
    VietnamVisaLog.step(name);
    await fn();
    result.filled += 1;
  } catch (err) {
    VietnamVisaLog.stepError(name, err);
    const message = err instanceof Error ? err.message : String(err);
    result.errors.push(`${name}: ${message}`);
  }
}

async function fillPersonal(data: VisaProfile, result: FillResult) {
  const p = (data.personal_information || {}) as ProfileSection;
  ...
  if (p.date_of_birth_mode) {
    await runStep('date_of_birth_mode', async () => {
      ...
      const radio = container.querySelector(`.ant-radio-input[value="${val}"]`) as HTMLInputElement;
  ...
  if (p.agree_create_account !== undefined) {
    await runStep('agree_create_account', () => H.fillCheckboxByText('Agree to create account by email', Boolean(p.agree_create_account)), result);
  }
  ...
  if (p.used_other_passports !== undefined) {
    await runStep('used_other_passports', () => H.fillRadioByQuestion('Have you ever used any other passports', Boolean(p.used_other_passports)), result);
  }
  if (p.multiple_nationalities !== undefined) {
    await runStep('multiple_nationalities', () => H.fillRadioByQuestion('Do you have multiple nationalities', Boolean(p.multiple_nationalities)), result);
  }
...
async function fillRequested(data: VisaProfile, result: FillResult) {
  const r = (data.requested_information || {}) as ProfileSection;
  const H = FieldHelpers;

  if (r.entry_type) {
    await runStep('entry_type', () => H.fillRadioByName('#basic_nddnTtDeNghi', String(r.entry_type)), result);
  }
...
async function fillPassport(data: VisaProfile, result: FillResult) {
  const p = (data.passport_information || {}) as ProfileSection;
  const H = FieldHelpers;
...
async function fillContact(data: VisaProfile, result: FillResult) {
  const c = (data.contact_information || {}) as ProfileSection;
  const e = (c.emergency_contact || {}) as ProfileSection;
  const H = FieldHelpers;
...
async function fillOccupation(data: VisaProfile, result: FillResult) {
  const o = (data.occupation || {}) as ProfileSection;
  const H = FieldHelpers;
...
async function fillTrip(data: VisaProfile, result: FillResult) {
  ...
  if (t.temporary_residence_commitment !== undefined) {
    await runStep('temporary_residence_commitment', () => H.fillCheckboxById('basic_ttcdCqTcCamDoan', Boolean(t.temporary_residence_commitment)), result);
  }
  if (t.contact_agency_in_vietnam !== undefined) {
    await runStep('contact_agency_in_vietnam', () => H.fillRadioByQuestion('Agency/Organization/Individual that the applicant plans to contact', Boolean(t.contact_agency_in_vietnam)), result);
  }
  if (t.visited_vietnam_last_year !== undefined) {
    await runStep('visited_vietnam_last_year', () => H.fillRadioByQuestion('Have you been to Viet Nam in the last 01 year', Boolean(t.visited_vietnam_last_year)), result);
  ...
  if (t.relatives_in_vietnam !== undefined) {
    await runStep('relatives_in_vietnam', () => H.fillRadioByQuestion('Do you have relatives who currently reside in Viet Nam', Boolean(t.relatives_in_vietnam)), result);
  }
}
...
async function fillChildren(data: VisaProfile, result: FillResult) {
  const children = (data.accompanying_children || []) as ProfileSection[];
  const H = FieldHelpers;
  if (!children.length) {
  ...
  for (let i = 0; i < children.length; i += 1) {
    ...
    if (child.sex && selects[0]) {
      await runStep(`${prefix}_sex`, async () => {
        const input = selects[0] as HTMLInputElement;
        const id = input.id || `child_sex_${i}`;
    ...
    if (child.date_of_birth && dateInputs[0]) {
      await runStep(`${prefix}_dob`, async () => {
        ...
        nativeInputValueSetter.call(input, child.date_of_birth);
...
async function fillExpenses(data: VisaProfile, result: FillResult) {
  const e = (data.trip_expenses || {}) as ProfileSection;
  const H = FieldHelpers;
...
async function fillDeclarations(data: VisaProfile, result: FillResult) {
  const d = (data.declarations || {}) as ProfileSection;
  const H = FieldHelpers;
  if (d.final_declaration !== undefined) {
    await runStep('final_declaration', () => H.fillCheckboxByText('I hereby declare that the above statements are true', Boolean(d.final_declaration)), result);
  }
}

export async function fillForm(profile: VisaProfile): Promise<FillResult> {
  VietnamVisaLog.info('fillForm started');
  const result: FillResult = { filled: 0, skipped: [], errors: [] };
  ...
  VietnamVisaLog.info('fillForm completed', {
  ...
  if (result.errors.length) {
    VietnamVisaLog.warn('fillForm errors', result.errors);
  }
